...
#     print(widthTop,widthBottom,heightTop,heightBottom)
#     print(points)
    return points
...
def getHistogram(img, minPercent=0.1, region=1, display=False):
    if region == 1: histValues = np.sum(img, axis=0)
    else: histValues = np.sum(img[img.shape[0]-(img.shape[0]//region):,:], axis=0)
